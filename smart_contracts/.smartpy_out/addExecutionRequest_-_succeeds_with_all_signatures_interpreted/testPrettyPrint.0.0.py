...
class Contract(sp.Contract):
  ...
  def addExecutionRequest(self, params):
    ...
    sp.for operator_public_key in self.data.operator_public_keys:
      sp.verify(match_pair_77_fst.contains(sp.hash_key(operator_public_key)), message = 'BAD_OP')
      sp.verify(sp.check_signature(operator_public_key, match_pair_77_fst[sp.hash_key(operator_public_key)], sp.pack(match_pair_77_snd)), message = 'BAD_SIG')
  ...
  def execute(self, params):
    match_pair_114_fst, match_pair_114_snd = sp.match_tuple(self.data.timelock[params], names = [ "match_pair_114_fst", "match_pair_114_snd" ])
    sp.verify(sp.add_seconds(match_pair_114_fst, sp.to_int(self.data.timelock_seconds)) < sp.now, message = 'TOO_EARLY')
    del self.data.timelock[params]
    sp.set_type(match_pair_114_snd(sp.unit), sp.TList(sp.TOperation))
    sp.for op in match_pair_114_snd(sp.unit):
