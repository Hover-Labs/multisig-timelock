   ...
   # TODO(keefertaylor): Consistent indentation and casing.
    ...
    def addExecutionRequest(self, signedKeyRotationRequest):
      ...
      with sp.for_('operator_public_key', self.data.operator_public_keys) as operator_public_key:
        ...
        with sp.if_(signatures.contains(keyHash)):
          with sp.if_(sp.check_signature(operator_public_key, signatures[keyHash], sp.pack(keyRotationRequest))):
    ...
    def addExecutionRequest(self, signedExecutionRequest):
      ...
      with sp.for_('operator_public_key', self.data.operator_public_keys) as operator_public_key:
        ...
        keyHash = sp.hash_key(operator_public_key)
        with sp.if_(signatures.contains(keyHash)):
          with sp.if_(sp.check_signature(operator_public_key, signatures[keyHash], sp.pack(executionRequest))):
            validSignaturesCounter.value += 1
